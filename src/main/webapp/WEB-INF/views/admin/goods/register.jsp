...
<head>
	...
	<style type="text/css">
		...
		.uploadResult ul{
			display: flex; 
			flex-flow: row; 
			justify-content: center; /* 水平の位置整列値 設定  */
			align-items: center; /* 垂直方向位置整列値 設定  */
		}
    ...
    <div id="wrapper">
	 ...
	 <jsp:include page="../include/menu.jsp" /> 
        ...
        <div class="container-fluid">
            ...
            <!-- Page Heading -->
            <h1 class="h3 mb-2 text-gray-800">商品登録</h1><br>
						...
						<form role="form" action="/admin/goods/register" method="post" enctype="multipart/form-data">
							<div class="form-group row">
							    <label class="col-md-1 col-form-label">分類</label>
		 				    	<select class="custom-select col-md-1 col-sm-4 category1" style="margin-right:12px; margin-left:12px;" required>
							  ...
							  <div class="form-group row">
							    <label class="col-md-1 col-form-label">商品名</label>
							      <div class="col-md-6">
							      <input type="text" name="gdsName" class="form-control" required>
							    </div>
							  </div>
							  ...
							  <div class="form-group row">
							    ...
							    <div class="col-md-2">
							      <input type="text" name="gdsPrice" class="form-control" pattern="[0-9]+" required>
							    </div>
							...
							<div class="form-group col-md-12 row">
								<label>商品説明</label> <textarea class="form-control" name="gdsDes" rows="10" required></textarea>
							</div>
							...
							<div style="float:right;">
								...
								<button type="button" class="btn btn-danger" onclick="location.href='/admin/goods/list'" style="margin-right:24px">キャンセル</button>										
							</div>	
						</form>
					</div>
				</div>
	...
	$(document).on("change", "select.category1", function(){
		 ...
		 */
		 ...
		 $("option:selected", this).each(function(){
		  ...
		  cate2Select.append("<option value=''>全体</option>");
	...
	$(document).ready(function(e){
		...
		$("input[type='file']").change(function(e){
			...
			$.ajax({
				...
				success: function(result){
					console.log(result);
					showUploadResult(result); //アップロード 結果処理関数
				}
			}); //$.ajax
		});
		...
		function showUploadResult(uploadResultArr){
			...
			$(uploadResultArr).each(function(i,obj){
				//img type
				obj.uploadPath = obj.uploadPath.replaceAll('\\','/');
				if(obj.image){
