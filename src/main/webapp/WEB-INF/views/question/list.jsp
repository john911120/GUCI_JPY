    ...
    <div class="container">
      <div class="row">
        <div class="col-lg-9 col-md-9 col-sm-8 col-md-push-3">
		  <div class="container">
			<div class="row">
				<h2 style="margin:20px 15px;">問い合わせ</h2>
				<div class="col-md-9">
					<c:if test="${user != null }">
					  <button class="btn btn-primary pull-right" type="button" onclick="location.href='/question/register'">記事を書く</button>
					</c:if>
				  <h3>問い合わせの履歴</h3>
				  <table class="table table-hover">
				    <thead>
				      <tr>
				        <th class="col-md-2">分類</th>
				    ...
				    <tbody>
						...
						<c:if test="${user.userId == ques.quesWri || user.userName == 'アドミン'}">
							<tr>
							<td><c:out value="${ques.quesCateCode }" /></td>
 							<td>
								<a class='move' href='<c:out value="${ques.quesNo}"/>'>
								<c:out value="${ques.quesTit }" />
								<b>[ <c:out value="${ques.replyCnt}" /> ] </b>
								</a>
							</td> 
										
							<td><fmt:formatDate pattern="yyyy-MM-dd"
									value="${ques.quesDate}" /></td>
			...
			<form id='searchForm' action="/question/list" method='get'>
							<select name='type'>
								...
								<option value="C"
								<c:out value="${pageMaker.cri.type eq 'C' ?'selected':''}"/>>コンテンツ</option>
								<option value="W"
								<c:out value="${pageMaker.cri.type eq 'W' ?'selected':''}"/>>作成者</option>
								<option value="TC"
								<c:out value="${pageMaker.cri.type eq 'TC' ?'selected':''}"/>>タイトル or コンテンツ</option>
								<option value="TW"
								<c:out value="${pageMaker.cri.type eq 'TW' ?'selected':''}"/>>タイトル or 作成者</option>
								<option value="TWC"
								<c:out value="${pageMaker.cri.type eq 'TWC' ?'selected':''}"/>>タイトル or コンテンツ or 作成者</option>
							</select>
			...
			<div class="modal fade" id="myModal" tabindex="-1" role="dialog"
				...
				<div class="modal-dialog">
					<div class="modal-content">
						...
						<div class="modal-body">処理しました。</div>
  ...
  <script type="text/javascript" src="/resources/js/slick.js"></script>
		...
		function checkModal(result) {
			...
			if(parseInt(result) > 0) {
				$(".modal-body").html("掲示記事 " + parseInt(result) + " 番が 登録しました。");
			}
			$("#myModal").modal("show");
		}
